"use strict";
var RecordedEmployment = new (function() {
  var self = this;
  self.municipalities = [];
...
//            console.log(self.months[keys[len - 1]]);
  ...
  self.init = function(initialData) {
    ...
    for (var i = 1; i < jlength; i++) {
      data = json.data[i];
      var m = self.findOrCreateMunicipality(self, data[0]);
      var p = new Period(data[1]);
      p.data = new MonthData();
      p.data.general = data[2];
      p.data.farming = data[3];
      p.data.house = data[4];
      p.data.sea = data[5];
      p.data.coal = data[6];
      p.data.freelance = data[7];
      p.data.total = data[8];
      m.periods.push(p);
    }
    return self;
